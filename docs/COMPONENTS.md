...
| **Qwen VL (hosted)** — [`qwen/qwen3.8-27b:free`](https://openrouter.ai/qwen/qwen3.8-27b) via OpenRouter | A newer, free-tier Qwen vision-language model, hosted | Automatic fallback backend — see `docs/ARCHITECTURE.md` §2 for why this is the fallback (not DashScope, blocked by account-country availability) and its current best-effort reliability status under OpenRouter's zero-payment-history throttling. |
